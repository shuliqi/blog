...
$jishusql = "select * from jishu order by id desc limit 6";
...
if ($anliresult && mysql_num_rows($anliresult)) {
...
<div class="template">
  <div class="box">
    ...
    <ul>
     ...
     ?>    
      <li><a href="article.show.php?id=<?php echo $value['id']; ?>&type=<?php echo  $value['type'] ;?>"><img src="admin/<?php echo $value['pic'];?>" alt="img14"/></a><span><?php echo $value['title'];?></span></li>
     <?php
         }
        }
      ?>    
...
<article>  
<div class="blog">
  <h3><p>最新文章<span>技术 Blog</span></p></h3>
  <?php 
      if (empty($webdata)) {
           echo "当前没有文章";
      }else{
  ...
  <div class="bloglist">
      <h2>
        <a href="article.show.php?id=<?php echo $value['id']; ?>&type=<?php echo  $value['type'] ;?>"><?php echo $value['title'];?></a>
      </h2>
